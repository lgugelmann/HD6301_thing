...
class Cpu6301 {
 ...
 protected:
  ...
  uint8_t get(uint16_t address) { return memory_->get(address); }

  uint16_t get16(uint16_t address) { return memory_->get16(address); }

  void set(uint16_t address, uint8_t data) { memory_->set(address, data); }

  void set16(uint16_t address, uint16_t data) { memory_->set16(address, data); }

  uint16_t get_d() { return (uint16_t)a << 8 | (uint16_t)b; }
  ...
  //
  void mem_op(uint16_t address, void (Cpu6301::*op)(uint8_t&),
              bool do_set = true) {
  ...
  void tap() { sr = StatusRegister(a); }

  void tpa() { a = sr.as_integer(); }
...
#define OP(expr) [this](uint16_t d) { expr; }
    static std::map<uint8_t, Instruction> instructions = {
        ...
        // AIM / OIM / EIM / TIM
        {0x61,
         {"aim", 3, 7, kIMM, OP(logic_m(x + fetch(), d, std::bit_and()))}},
        ...
        {0x65,
         {"eim", 3, 7, kIMM, OP(logic_m(x + fetch(), d, std::bit_xor()))}},
        {0x75, {"eim", 3, 6, kIMM, OP(logic_m(fetch(), d, std::bit_xor()))}},
        {0x6b,
         {"tim", 3, 7, kIMM,
          OP(logic_m(x + fetch(), d, std::bit_xor(), false))}},
        {0x7b,
